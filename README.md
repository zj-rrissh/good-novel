...
| 模块 | 完成度 | 核心能力 |
|------|:------:|---------|
| 用户中心 | 70% | 注册、登录、JWT 刷新、资料管理、消息通知 |
| 阅读模块 | 65% | 小说详情、章节分页、阅读进度、缓存加速 |
| 小说管理 | 60% | 创建/编辑小说与章节、提审、上架/下架 |
| 缓存与并发 | 70% | 二级缓存、空值防护、互斥锁、分布式锁、幂等 |
| 持久化 | 55% | 核心表结构、MyBatis Mapper、Flyway 迁移 |
| 审核引擎 | 45% | 审核受理、人工复审、状态回写 |
| 基础设施 | 50% | TraceId、访问日志、异常处理、定时任务、Actuator |
| 用户接入层 | 40% | 统一响应、路径契约、客户端识别 |
| 安全治理 | 35% | JWT Bearer、角色放行、限流/风控待闭环 |
| 社区互动 | 25% | 评论创建/删除（查询待真实化）、点赞/关注待实现 |
| 推荐引擎 | 20% | 接口就绪，当前返回示例数据 |
| 管理端 | 35% | 作者创作主链路可走通，管理查询待真实化 |
...
| `AINOVEL_DB_PASSWORD` | `ainovel123` | 数据库密码（敏感） |
| `AINOVEL_REDIS_HOST` | `127.0.0.1` | Redis 主机 |
| `AINOVEL_REDIS_PORT` | `6380` | Redis 端口 |
| `AINOVEL_REDIS_PASSWORD` | — | Redis 密码（敏感） |
| `AINOVEL_JWT_SECRET` | — | JWT 签名密钥（敏感，生产必设） |
