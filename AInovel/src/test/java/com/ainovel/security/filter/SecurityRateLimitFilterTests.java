...
@TestPropertySource(properties = {
        ...
        "ainovel.security.rate-limit-auth-threshold=1",
        "ainovel.security.rate-limit-auth-window-seconds=60",
        "ainovel.security.risk-control-enabled=false"
})
class SecurityRateLimitFilterTests {

    @Autowired
    private MockMvc mockMvc;

    @Test
    void shouldBlockSecondAuthRequestWithRateLimitedCode() throws Exception {
        String payload = """
                {
                  "username": "missing_user",
                  "password": "wrong_password",
                  "deviceId": "device-rate-1"
                }
                """;
